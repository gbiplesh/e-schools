<?php
	...
	if(isset($_POST["submit"])) {
		...
		$conn = mysql_connect("localhost", "eschool","password");
				
				if (!$conn) {
					die($conn->connect_error);
				}

				mysql_select_db ("eschool", $conn);
	
			$result = mysql_query("SELECT * FROM admins WHERE username = '$username' AND legal = '1'") or die("Failed to query database".mysql_error());
